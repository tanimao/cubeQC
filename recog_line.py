...
def detect_HoughCircles(saveName,img,img2, col):
    ...
    ret,th = cv2.threshold(gray2,180,255,cv2.THRESH_BINARY)    

    minVal = 100
    maxVal = 200
    edges = cv2.Canny(gray2, minVal, maxVal)
...
#            print('center: ({}, {}), radius: {}'.format(x, y, radius))
        ...
        for i, xyr in enumerate(circles):
            ...
            #x_ave = (i*x_ave + xyr[0])//(i+1)
            #y_ave = (i*y_ave + xyr[1])//(i+1)
            #r_ave = (i*r_ave + xyr[2])//(i+1)
            x_ave = xyr[0]
            y_ave = xyr[1]
            r_ave = xyr[2]
...
#    plt.imshow(cv2.cvtColor(img2, cv2.COLOR_BGR2RGB))
    ...
    np.save('files/'+saveName+'.npy', hist_mask)
...
def get_PixelValues(saveName, img, col, cube_size):
   ...
   # myax2 = myfig.add_subplot(3,1,2), plt.imshow(masked_gray, 'gray')
    ...
    np.save('files/'+saveName+'_whole.npy', hist_mask)
    ########################################
...
def hough_lines(image, outLineImage):
    ...
    for line in lines:
        ...
        for rho, theta in line:
            ...
            cv2.line(result,(x1,y1),(x2,y2),(0,255,0),2) # 緑色で直線を引く
        '''
        print(line)
        for  x1, y1, x2, y2 in line :
            ...
            xbar.append(xlocal)
            ybar.append(ylocal)
            print("xbar is {}, ybar is {}.\n".format(xlocal,ylocal))
    ...
    fig = plt.figure(figsize=(8, 6), dpi=100)
    plt.imshow(edges)
    xylist = [xmax, xmin, ymax, ymin, ymaxnew]
    return xylist
...
def main():
    ...
    saveName  = 'good58_1' 
    ImageName = 'mao_pictures/pic1008good/'+ saveName + '.jpg' 
...
#    cube_size = detect_Outline_wColFigure(result2, 1, 500)
    ...
    file = open('files/cube_data.txt', 'a')
    #file.write(ImageName +' '+ str(circleInfo[0])+' '+str(circleInfo[1])+' '+str(circleInfo[2])+' '+str(cube_size[0])+' '+str(cube_size[1])+'\n')
    file.write(ImageName +' '+ str(circleInfo[0])+' '+str(circleInfo[1])+' '+str(circleInfo[2])+' '+str(xsizeeff)+' '+str(ysizeeff)+'\n')
    file.close()
    plt.show()
