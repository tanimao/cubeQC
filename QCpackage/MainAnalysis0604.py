...
'''
###cubeQC.py で使う時はこちらを使う。
from QCpackage.package_cube import manualminimize
from QCpackage.package_cube import figureInfo
from QCpackage.package_cube import resizeFigure
from QCpackage.package_cube import extractionRGB 
from QCpackage.package_cube import trimmFig
from QCpackage.package_cube import houghCircle
from QCpackage.package_cube import searchCenter
from QCpackage.package_cube import outline
from QCpackage.package_cube import pixVal
from QCpackage.package_cube import houghLine2
from QCpackage.package_cube import findThres
from QCpackage.package_cube import detectHole
from QCpackage.package_cube import outline2
from QCpackage.package_cube import choiceOutline3
...
def ANALYSIS(saveName, ImageName):
    start = time.time()
    for cubenumber in range(16):
        ...
        for i in range(6):
            for k in range(3):
                ...
                for j in range(4):
                    #saveName  = 'cube1cam3_12' 
                    saveName  = 'cube'+str(cubenumber+1)+'cam'+str(k+1)+'_'+str(i+1)+str(j+1)
                    ImageName = 'mao_pictures/pic0820daiza3/'+ saveName + '.jpg' 
                 #   print (saveName)   
...
###############deeplearning用の画像生成########## サイズを 660*60　に固定
            ...
            #バンプ検出、どう扱うか。       
                    ...
                    file2.close()
                    '''
                    cubedata=[round(xc,2), round(yc,2), round(opt_r,2), round(Emin,2), round(xsizeeff,2), round(ysizeeff,2), round(bump_judge, 5)]#, directionID]
                    print('saveName:{} ,output:{}\n'.format(saveName, cubedata))
                    file = open('files/data0825daiza3.txt','a')
                    file.write(saveName +' icam: '+str(k+1)+' direction: '+str(directionID+1)+' '+str(cubedata)+' size(outline) :'+str(round(cube_size[0],2))+','+str(round(cube_size[1],2))+'\n')
                    file.close()
